...
const lowercaseDrugNames = drugs.map((drug) => drug.name.toLowerCase());
console.log('Lowercase drug names:', lowercaseDrugNames);

// 3. Write a function that accepts a category and returns all drugs under that category.
const getDrugsByCategory = (category) => {
  const categorizedDrugs = drugs.filter((drug) => drug.category === category);
...
console.log('Formatted drug information:', formattedDrugs);
...
const nonPrescriptionDrugs = drugs.filter(
  (drug) => drug.isPrescriptionOnly !== true
);
...
function countDrugsByManufacturer(manufacturer) {
  ...
  return manufacturerDrugs.length;
}
console.log('Number of drugs by Pfizer:', countDrugsByManufacturer('Pfizer'));
